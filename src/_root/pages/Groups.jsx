...
const Groups = ({
  ...
  useEffect(() => {
    ...
    if (storedUser && storedUser.email != "") {
      ...
      // Save user details to localStorage if not already saved
  ...
  return (
    <div
      className={`fixed flex-col w-80 bg-bg h-[100vh] shadow p-8 pt-6 justify-between overflow-y-auto ${isSidebarVisible ? "w-screen flex z-50" : "hidden lg:flex"}`}
    >
      <div>
        ...
        >
          <div className="h-14 w-14 rounded-full bg-grey2"></div>
        ...
        <h2 className="text-xs font-normal text-grey2 mb-3">
          ...
          {groups.map((group) => {
            ...
            return (
              <li key={groupId} className="group">
                <NavLink
                  onClick={() => {
                    if (isActive) setSidebarVisible(false);
                  }}
                  to={`/groups/${groupId}`}
        ...
        <button
          ...
          className="group flex items-center gap-2 w-full my-3"
        >
          {isCreatingGroup ? (
            <p className="flex bg-red-500 py-1 px-2 mt-1 rounded text-white font-bold">
              Cancel
            </p>
          ) : (
            <>
              ...
              />
              <p className="text-accent1 text-lg font-semibold opacity-80 group-hover:opacity-100 transition">
                Create New Group
              </p>
            </>
          )}
